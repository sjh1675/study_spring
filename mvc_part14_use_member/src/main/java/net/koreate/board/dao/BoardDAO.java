...
//	@Insert("INSERT INTO re_tbl_board(title, content, uno) VALUES(#{title}, #{content}, #{uno})")
	...
	// 가장 최근에 등록된 기본키값으로 origin column 값 수정
	...
	/*
	 * @Select("SELECT R.*, U.uname AS writer FROM re_tbl_board AS R NATURAL JOIN tbl_user AS U ORDER BY R.origin DESC, R.seq ASC limit #{startRow}, #{perPageNum}"
	 * )
	 */	
...
//	@Select("SELECT count(*) FROM re_tbl_board")
	// 검색 결과 내에서 전체 게시글 목록 갯수
	@SelectProvider(type=BoardQueryProvider.class, method="searchSelectCount")
	int listCount(SearchCriteria cri) throws Exception;
	
	// bno가 일치하는 게시글 정보
	@Select("SELECT R.*, U.uname AS writer FROM re_tbl_board AS R NATURAL JOIN tbl_user AS U WHERE R.bno = #{bno}")
