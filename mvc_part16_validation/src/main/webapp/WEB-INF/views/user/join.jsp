...
<script>
	...
	// 메세지를 보여줄 요소(위치), 검사값, 비교식, 출력할 메세지, 비동기 통신함수
	function checkRegex(elP, valP, regexP, messageP, ajaxP) {
		if(regexP.test(valP) === false) {	// 정규식 검사 불일치
			showMessage(elP, messageP, false);
			return false;
		} else if (regexP.test(valP) !== false && ajaxP === null) {	// 일치하고 비동기통신 함수가 없다면
	...
	function checkUidAjax(elP) {
		$.ajax({
			...
			success : function(result) {
				console.log(result);
				boolUid = result;
				showMessage(elP, boolUid ? "사용가능한 아이디입니다." : "존재하는 아이디", boolUid);
				
				if (boolUid) {	// 사용가능한 계정이면 인증코드 발송 버튼 활성화
					$("#acceptEmail").show();
				} else {
					$("#acceptEmail").hide();
				}
