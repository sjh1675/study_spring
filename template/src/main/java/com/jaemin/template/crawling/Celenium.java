...
//			System.out.println("파일 생성 경로 : " + file.getAbsolutePath());
	...
	public List<TempVO> crawling(WebDriver driver, String url, String imgFolderPath) throws Exception {
		...
		for (String newUrl : urlList) {
			...
			String pubdate = titleAll.substring(titleAll.length() - 10, titleAll.length());
			...
			TempVO vo = new TempVO(code, title, auth, pub, pubdate, pageCnt, genre, intro);
